...
// cicli for o do in modo diretto schiantano... v. nota sotto getNextStatement
...
enum __attribute((packed)) {
  ...
  FIRST_CHAR_FUNCTION=CHRSTRING,
	...
	HEXSTRING,
	TRIMSTRING,
	TABSTRING,    // la inserisco qua ma SENZA $
	TIMESTRING,
	INKEYSTRING,
	GETKEYSTRING,
	MIDISTRING,
...
BSTATIC TOKEN_LIST const tl[] = {
  ...
  { "POWER",5 },
	...
	{ "CLOSE",5 },
	{ "AS",2 },
	{ "DIR",6 }, { "CHDIR",5 }, { "MKDIR",5 }, { "RMDIR",5 }, { "RENAME",6 }, { "DELETE",6 }, { "FORMAT",6 },
	{ "CLS",3 }, { "LOCATE",6 },
...
BSTATIC LINE_NUMBER_STMT_POS_TYPE findData(MINIBASIC *);
...
BSTATIC void doLPrint(MINIBASIC *);
...
BSTATIC void doData(MINIBASIC *);
BSTATIC void doRead(MINIBASIC *);
BSTATIC void doRestore(MINIBASIC *);
...
BSTATIC LINE_NUMBER_STMT_POS_TYPE doRun(MINIBASIC *);
...
BSTATIC void doTrace(MINIBASIC *);
...
BSTATIC int subLoad(MINIBASIC *,const char *);

BSTATIC int16_t integerExpr(MINIBASIC *);
...
BSTATIC int16_t integer(MINIBASIC *,NUM_TYPE x);

BSTATIC void match(MINIBASIC *,TOKEN_NUM);
BSTATIC void setError(MINIBASIC *,enum INTERPRETER_ERRORS errorcode);
BSTATIC LINE_NUMBER_TYPE getNextLine(MINIBASIC *,const char *);
BSTATIC LINE_NUMBER_STMT_POS_TYPE getNextStatement(MINIBASIC *,const char *);
BSTATIC TOKEN_NUM getToken(const char *str);
BSTATIC TOKEN_NUM getAToken(MINIBASIC *);
...
static uint16_t myStrCount(const char *str, char ch);
static char *myStrConcat(const char *str, const char *cat);
BSTATIC double factorial(double x);

BSTATIC char myitoabuf[17];
BSTATIC inline __attribute((always_inline)) const char *skipSpaces(const char *);
int8_t strnicmp(const char *, const char *, size_t);
const char *strichr(const char *, const char);
...
double myround(double d,double n);
...
//------------------------------------------------------------- Send CopyRight Message ----------------------------------------------------------
  hInstance->script=NULL;
  hInstance->incomingChar[0]=hInstance->incomingChar[1]=0;
  hInstance->errorFlag=ERR_CLEAR;
  hInstance->readDataPtr.line=0; hInstance->readDataPtr.pos=0; 
  hInstance->trace=0;
#ifdef USA_BREAKTHROUGH
  HDC hDC;
...
//            myTextOut(hInstance,cmdPointer);
            ...
            if(!strnicmp(commandLine,"RUN",3)) {
              ...
              if(*parmsPointer=='\"') {
                ...
                i=basic(hInstance,buf,1);
  //                if(!i)
  //                  break;
                if(hInstance->script) {
                  free(hInstance->script);
                  hInstance->script=NULL;
                  }
                goto ready2;
                }     // se nomefile..
...
execline:
                ...
                hInstance->token = getAToken(hInstance);
                hInstance->errorFlag = ERR_CLEAR;
...
//	setTextColorBG(textColors[mInstance->ColorPalette],textColors[mInstance->ColorPaletteBK]); 
  ...
  if(where) {
    switch(subLoad(mInstance,(char *)script)) {
      case -1:
        myTextOut(mInstance,"Program not found");
        myCR(mInstance);
        goto end_all;
        break;
      case -2:
        myTextOut(mInstance,"Not enough memory");
        myCR(mInstance);
        goto end_all;
        break;
      default:
    ...
    mInstance->script=malloc(strlen(script)   +2);
    if(!mInstance->script) {
end_nomemory:
      answer=-1;										//Script error END
      goto end_all;
      }
    strcpy(mInstance->script,script);
    }

  if(setup(mInstance,(char *)mInstance->script) == -1) {				//Run setup Function to extract data from the Script
    answer=-1;										//Script error END
    goto end_all;
    }
  ...
  mInstance->string = mInstance->lines[mInstance->curline].str;
    ...
    switch(mInstance->trace & 0x83) {
      char buf[16];
			case 1:
				sprintf(buf,"[%u]",mInstance->lines[mInstance->curline].no);
				myTextOut(mInstance,buf);
				break;
			case 2:		// a volte potrebbe andare meglio questo...
				err_printf("[%u]",mInstance->lines[mInstance->curline].no);
				break;
			case 0x81:
				sprintf(buf,"[%u[%u]]",mInstance->lines[mInstance->curline].no,
					mInstance->string-mInstance->lines[mInstance->curline].str);
				myTextOut(mInstance,buf);
				break;
			case 0x82:		// idem
				err_printf("[%u[%u]]",mInstance->lines[mInstance->curline].no,
					mInstance->string-mInstance->lines[mInstance->curline].str);
				break;
      }
...
BSTATIC LINE_NUMBER_STMT_POS_TYPE findData(MINIBASIC *mInstance) {
  const char *p;
  int i;
	TOKEN_NUM t;
...
void doPrint(MINIBASIC *mInstance) {
  char *str,*strUsing=NULL,*strUsingPtr=NULL;
  NUM_TYPE x;
	void *ftemp=NULL;
  enum _FILE_TYPES filetype=-1;
	uint8_t pendingCR=0;                 // per semicolon a fine riga
  char buf[MAXSTRINGLEN+1],*pbuf;
  enum DATATYPE typeOfVar;
  

  match(mInstance,mInstance->token);		// e PRINT2 

  if(mInstance->token == DIESIS)	{	// per print to file
		unsigned int f,i;
    
		match(mInstance,DIESIS);
    
		f=integerExpr(mInstance);
...
print_file_found:
  if(mInstance->token == USING)	{
    match(mInstance,USING);
    strUsingPtr=strUsing=stringExpr(mInstance);	//https://www.qbasic.net/en/reference/qb11/Statement/PRINT-USING.htm
...
print_outbuf: 
          pbuf=buf;
          if(strUsingPtr) {
            char *dotpos=NULL,*exppos=NULL,/**signpos,*/filler=' ';
            uint8_t buflen,declen=0,filllen,formatlen,align,showsign=0;
            align=0 /*left, qbasic di default fa right..*/;
            if(strchr(strUsing,'.')) {  // 
              align=1;    // direi
              }
            if(typeOfVar==FLTID || typeOfVar==INTID) {
              if(*pbuf==' ')    // rimuovo lo spazio dei positivi (gestito poi)
                memcpy(buf,buf+1,strlen(buf));    // incluso lo 0
              }
            if(typeOfVar==FLTID) {
              if(dotpos=strchr(strUsing,'.')) {  // calcolo dec richiesti
                declen=strlen(dotpos+1);
                if(strchr(dotpos,'^'))    // se c'era in mezzo, lo sottraggo!
                  declen--;
                }
              if(strchr(strUsing,'^')) {  // 
                char *p;
                ...
                double mantissa;
                align=1;    // direi
                exponent=strchr(strUsing,'+') ? 1 : 0;    // boh... non so..
...
//                  exponent = (int)log10(fabs(x));
                  p=strchr(strUsing,'#');
                  if(!p) p=strUsing; // DEVE esserci! ma ok :D
                  while(*p++=='#')
                    exponent++;
...
//                sprintf(buf,"%fE%d", mantissa, exponent);
              ...
              if(dotpos=strchr(buf,'.')) {  // tolgo il punto 
                *dotpos=0;
                }
              //(...cmq salvo pos per dopo, in caso)
              }
            buflen=strlen(pbuf) + (declen ? (declen+1) : 0);
            formatlen=strlen(strUsing) - (strchr(strUsing,'^') ? 1 : 0);
            filllen=formatlen>buflen ? formatlen-buflen : 0;
            do {
              switch(*strUsingPtr) {
                case '#':
copy_field:
                  ...
                  if(buflen>0) {
                    if((showsign) && (pbuf==buf)) {          // se primo char...
                      showsign=0;
                      if(*pbuf=='-') {
//                        *pbuf='-';  // :)
                        *strUsingPtr=*pbuf++;
...
put_filler:
            ...
            // se il campo è troppo piccolo, qbasic antepone un % e NON tronca.. (io tronco ma piuttosto metterei tutti char di errore)
                  break;
                case '+':
                  if(typeOfVar==INTID || typeOfVar==FLTID) {
                    showsign=1;
                    if(buf[0] != '-')
                      buflen++;
                    goto rimuovi_char;
                  ...
                  break;
                case '.':
                  /*if(typeOfVar==FLTID)*/ {    // vale cmq, metto zeri
                    if(dotpos) {
                      pbuf=dotpos+1;
                      }
                    else {
                      }
//                    filler='0';   // in effetti non viene usato, v.sopra (gestisco là)
                    buflen=declen;
                    *strUsingPtr='.';
                    }
                  break;
                case ',':
                  if(typeOfVar==FLTID) {    // direi... e SOLO se prima del punto decimale...
                    // FORMATTO a migliaia ecc; v. char *formatNumber(unsigned long long n,char *out) {
                      goto copy_field;
                    }
                  break;
                case '^':
                  if(typeOfVar==FLTID) {  // finire
//                    buflen=0;   // NO! è ok se non c'è punto decimale e il campo è troppo piccolo.... (elimina errore "campo piccolo", a questo punto
                    *strUsingPtr++='E'; exppos++;
                    *strUsingPtr++=*exppos++;   // sono max 3 incluso term :)
                    *strUsingPtr++=*exppos++;
                    *strUsingPtr++=*exppos++;
                    *strUsingPtr=0;
                    goto fine_using;
                  ...
                  break;
                case '!':
                  if(typeOfVar==STRID)   // stampa solo 1 char e skippare
                    goto copy_field;
                  break;
                case '\\':    
                  if(typeOfVar==STRID) { // tra 2 backslash dovrebbe stampare n-spazi(+2) char di una stringa..
                    do {
                      *strUsingPtr++=*pbuf++;
                      buflen--;
                      } while(*strUsingPtr && *strUsingPtr!='\\');
                    if(*strUsingPtr=='\\')
                      *strUsingPtr=*pbuf;   // l'ultimo...
                    else    // se non c'è la chiusura andrebbe dato errore!
                      ;
                    buflen=0;   // bah elimina CMQ errore "campo piccolo"
                    goto fine_using;
                    }
                  break;
                case '&':
                  if(typeOfVar==STRID) {   // stampare tutta stringa e via!
                    strUsing=realloc(strUsing,formatlen+buflen);
                    strcpy(strUsingPtr,pbuf);
                    buflen=0;   // elimina errore "campo piccolo"
                    goto fine_using;
                    }
                  break;
                case '-':     // questo dovrebbe fare right-align...
//                    goto copy_field;    // ergo fare!!
                  align=1;
                  goto rimuovi_char;
                  break;
                case '*':     // sostituire spazi con asterischi.. leading o tutti o boh
                  filler='*';
                  goto rimuovi_char;
                  break;
                case '$':     // idem
                ...
                case '£':     // idem mio
                  ...
                  goto rimuovi_char;
                  break;
                case '_':     // stampo il char che segue, literal
                  memcpy(strUsingPtr,strUsingPtr+1,strlen(strUsingPtr));    // incluso lo 0
                  formatlen--; 
                  filllen=formatlen>buflen ? formatlen-buflen : 0;
                  break;
                default:
...
fine_using:
            pbuf=strUsing;
            if(buflen>0)    // se avanzano char ossia format troppo piccolo, indico così ERRORE
              pbuf[0]='%';
            }
...
#endif
						case FILE_DISK:
							if(SuperFileWrite(ftemp,pbuf,strlen(pbuf)) != strlen(pbuf)) {     // 
								setError(mInstance,ERR_FILE);
								}
							break;
						default:
							myTextOut(mInstance,pbuf);
							break;
						}
					pendingCR=1;
          
          if(strUsing)    // per ora solo una var con USING...
            goto fine;
			...
			case TABSTRING:
	  		match(mInstance,TABSTRING);
        match(mInstance,OPAREN);
        mInstance->Cursor.x=integerExpr(mInstance);
        if(mInstance->Cursor.x >= ScreenText.cx)
          mInstance->Cursor.x = ScreenText.cx-1;
#ifndef USING_SIMULATOR
...
/* questa era per una print using tipo printf... ma non è così che la vogliamo!
  in effetti QBASIC: accetta char extra e li stampa; dà errore se tipo var inadatto al format;
  * 
          pbuf=buf;
          if(strUsingPtr) {
            char *dotpos,*signpos,filler=' ';
            uint8_t buflen,filllen=0,formatlen,infield=0;
            buflen=strlen(buf);
            if(strUsingPtr=strchr(strUsing,'#')) {
              while(*strUsingPtr && (*strUsingPtr=='#' || *strUsingPtr=='.')) {   // ev. completare..
                formatlen++; strUsingPtr++;
                }
              }
            else
              formatlen=0;
            strUsingPtr=strUsing;
            dotpos=strchr(buf,'.');
            signpos=strchr(buf,'-');
            while(*strUsingPtr) {
              switch(*strUsingPtr) {
...
copy_field:
                ...
                case '+':
                  if(typeOfVar==INTID || typeOfVar==FLTID) {
                    if(pbuf==buf) {          // se primo char... [bah, provare!]
                      if(*pbuf==' ') {
                        *pbuf='+';
                        goto copy_field;
                        }
                ...
                case ',':
                  ...
                  break;
                case 'E':
                  if(typeOfVar==FLTID)   // finire, riformattando numero...
                    goto copy_field;
                  break;
                case '!':
                  if(typeOfVar==STRID)   // stampare solo 1 char e skippare
                    goto copy_field;
                  break;
                case '&':
                  if(typeOfVar==STRID)   // stampare tutta stringa e via!
                ...
                case '£':     // idem mio
                  ...
                  break;
                case '_':     // stampo il char che segue, literal
                  memcpy(strUsingPtr,strUsingPtr+1,strlen(strUsingPtr)-1);    // provare...
                  strUsingPtr--;
                  break;
                default:
                  infield=0;
...
void doLPrint(MINIBASIC *mInstance) {
  char *str,*strUsing=NULL,*strUsingPtr=NULL;
  NUM_TYPE x;
	uint8_t pendingCR=0;                 // per semicolon a fine riga
  char buf[MAXSTRINGLEN+1],*pbuf;
  enum DATATYPE typeOfVar;
  //si potrebbe avere qua una variabile X per la posizione orizzontale ma... alla fine funzionerebbe poco
  // o magari dentro sil south-bridge, idem...

  match(mInstance,LPRINT);		// 
...
//                    filler='0';   // in effetti non viene usato, v.sopra (gestisco là)
                ...
                case '^':
                  if(typeOfVar==FLTID) {  // finire
                    buflen=0;   // elimina errore "campo piccolo", a questo punto
                    *strUsingPtr++='E'; exppos++;
                    *strUsingPtr++=*exppos++;   // sono max 3 incluso term :)
                    *strUsingPtr++=*exppos++;
                    *strUsingPtr++=*exppos++;
                ...
                case '!':
                  ...
                  break;
                case '\\':    
                  if(typeOfVar==STRID) // tra 2 backslash dovrebbe stampare n char di una stringa..
                    ;
                  break;
                case '&':
                  if(typeOfVar==STRID) {   // stampare tutta stringa e via!
...
fine_using:
          ...
          if(strUsing)    // per ora solo una var con USING...
			...
			case TABSTRING:
	  		match(mInstance,TABSTRING);
        match(mInstance,OPAREN);
        integerExpr(mInstance);
        // mmm come fare?? v. nota variabili all'inizio..
        match(mInstance,CPAREN);
				break;
...
void doLocate(MINIBASIC *mInstance) {
  ...
  x = integerExpr(mInstance);
  match(mInstance,COMMA);
  y = integerExpr(mInstance);
  ...
  LINE_NUMBER_STMT_POS_TYPE answer;
    ...
    switch(mInstance->token) {
      case GOTO:
        ...
        mInstance->errorHandler.handler = integerExpr(mInstance);
        break;
      case CONTINUE:
    ...
    answer.d=STMT_CONTINUE;
		...
		getAToken(mInstance);
		match(mInstance,GOTO);
		mInstance->irqHandler.handler = integerExpr(mInstance);
		mInstance->irqHandler.line.d = 0;
    answer.d=STMT_CONTINUE;
		return answer;
		}
    ...
    TMR6=TMR7=0;
		match(mInstance,GOTO);
		mInstance->timerHandler.handler = integerExpr(mInstance);
		mInstance->timerHandler.line.d = 0;
    answer.d=STMT_CONTINUE;
		return answer;
		}
	else {
		n = integerExpr(mInstance);
		t=getAToken(mInstance);
		if(t==GOSUB) {
...
#endif
		if(!strnicmp(str,"LPT:",4)) {
			mInstance->openFiles[mInstance->nfiles].type=FILE_LPT;
      // finire! fare
//      SetSerialPort(9600,8,0,1,FALSE);
//    	InitUART((BYTE)(DWORD)mInstance->openFiles[mInstance->nfiles].handle,9600); 
      mInstance->nfiles++;
...
void doPoke(MINIBASIC *mInstance) {
  ...
  adr = integerExpr(mInstance);
  match(mInstance,COMMA);
  dat = integerExpr(mInstance);
...
rifo:
    ...
    setError(mInstance,ERR_OUTOFDATA);
    }
  
  }
  
BSTATIC void doRestore(MINIBASIC *mInstance) {
  int i;
  
  match(mInstance,RESTORE);
  if(!mInstance->script) {
    setError(mInstance,ERR_INVALIDDIRECT);
    return;
    }
...
/*
  the RUN statement
*/
LINE_NUMBER_STMT_POS_TYPE doRun(MINIBASIC *mInstance) {
  int t=0;
  LINE_NUMBER_STMT_POS_TYPE answer;
...
// RIPROVARE  #warning RUN non va... bisogna passare da commandline a basic() ...
...
void doNew(MINIBASIC *mInstance) {
  ...
  match(mInstance,NEW);
	*mInstance->script=0;   // beh inutile ..
...
int subLoad(MINIBASIC *mInstance,const char *s) {
  SUPERFILE f;
  int n=0;
  char *filename;
  char *p;
  ...
  if(SuperFileOpen(&f,filename,'r')) { 
    p=mInstance->script=malloc(SuperFileSize(&f,NULL)    +2);
    if(!mInstance->script) {
      SuperFileClose(&f);
      return -2;
      }
...
void doLoad(MINIBASIC *mInstance) {
  ...
  if(subLoad(mInstance,filename) < 0)
    setError(mInstance,ERR_FILE);
  else
    setup(mInstance,mInstance->script);
  free(filename);
	}
...
void doTrace(MINIBASIC *mInstance) {
  int i,n=0;
  
  match(mInstance,TRACE);
  mInstance->trace=integerExpr(mInstance);
	}
...
void doWebcam(MINIBASIC *mInstance) {
  ...
  sz.cx = integerExpr(mInstance);
  if(sz.cx < 0 || sz.cx > MAX_HORIZ_SIZE_WEBCAM)    //
		setError(mInstance, ERR_BADVALUE);
  match(mInstance,COMMA);
  sz.cy = integerExpr(mInstance);
...
void doSetpin(MINIBASIC *mInstance) {
  ...
  port = integerExpr(mInstance);
  match(mInstance,COMMA);
  pin = integerExpr(mInstance);
  match(mInstance,COMMA);
  mode = integerExpr(mInstance);
...
// LEGGERE WAVEFMT da file! FARE controlli!
    ...
    match(mInstance,COMMA);
    varsize=integerExpr(mInstance); // andrebbe controllata dimensione... che è stata malloc'ata...
    if(mInstance->token == COMMA) {
      match(mInstance,COMMA);
      wsf.loop = expr(mInstance);
      }
...
void doDim(MINIBASIC *mInstance) {
  ...
  switch(mInstance->token) {
    case DIMFLTID:
      ...
      getId(mInstance,mInstance->string, name, &len);
			match(mInstance,mInstance->token);
			dims[ndims++] = integerExpr(mInstance);
  ...
  if(mInstance->token == EQUALS) {
    match(mInstance,EQUALS);
      ...
      case INTID:
				i=0;
				dimvar->d.ival[i++] = integerExpr(mInstance);
				while(mInstance->token == COMMA && i < size)	{
					match(mInstance,COMMA);
					dimvar->d.ival[i++] = integerExpr(mInstance);
          if(mInstance->errorFlag != ERR_CLEAR)
...
LINE_NUMBER_STMT_POS_TYPE doIf(MINIBASIC *mInstance) {
  ...
  if(mInstance->token==VALUE) {   // GD 2022
    jumpthen.line=findLine(mInstance,integerExpr(mInstance));
    jumpthen.pos=0;
    if(jumpthen.line == -1) {
      setError(mInstance,ERR_NOSUCHLINE);   // (sopra è gestita a parte)
      }
    if(mInstance->token == ELSE) {
      ...
      jumpelse.line=findLine(mInstance,integerExpr(mInstance));
      jumpelse.pos=0;
      if(jumpelse.line == -1) {
        setError(mInstance,ERR_NOSUCHLINE);   // (sopra è gestita a parte)
        }
...
LINE_NUMBER_STMT_POS_TYPE doGoto(MINIBASIC *mInstance) {
  ...
  match(mInstance,GOTO);
 //if(!mInstance->script)  esci, v. getNextStatement, FARE!
  // setError(ERR_INVALIDDIRECT);
  // return
  
  return subGoto(mInstance,integerExpr(mInstance));
	}
...
void doError(MINIBASIC *mInstance) {
  ...
  match(mInstance,ERRORTOK);
  setError(mInstance,integerExpr(mInstance));
	}
...
get_file_found:
  switch(filetype) {
    ...
    case FILE_CDC:
      ...
      break;
#endif
    case FILE_LPT:    // ovviamente nulla qua!
      break;
#ifdef USA_WIFI
    case FILE_TCP:
      for(j=0; j<varsize; j++) {
...
put_file_found:
  ...
  switch(filetype) {
    ...
    case FILE_LPT:
      for(j=0; j<varsize; j++)
        WriteParallel(LOBYTE(dimvar->d.ival[j]));
      break;
#ifdef USA_WIFI
    case FILE_TCP:
      for(j=0; j<varsize; j++)
...
//#pragma warning( push )
//#pragma warning( disable : all)   // non c'è cazzi di levare il warning "noinline" PD
//__longramfunc__ (*ramfuncEntryPoint)();
__attribute__ ((ramfunc,section(".ramfunc"),far,unique_section)) (*ramfuncEntryPoint)();    // faccio così
//#pragma warning( pop ) 
...
void doLine(MINIBASIC *mInstance) {
  ...
  pt1.x = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt1.y = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt2.x = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt2.y = integerExpr(mInstance);
  if(mInstance->token == COMMA) {
    match(mInstance,COMMA);
    size=integerExpr(mInstance);
    }
  else
    size=1;
...
void doRectangle(MINIBASIC *mInstance) {
  ...
  pt[0].x = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt[0].y = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt[1].x = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt[1].y = integerExpr(mInstance);
...
void doTriangle(MINIBASIC *mInstance) {
  ...
  pt[3].x = pt[0].x = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt[3].y = pt[0].y = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt[1].x = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt[1].y = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt[2].x = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt[2].y = integerExpr(mInstance);
...
void doEllipse(MINIBASIC *mInstance) {
  ...
  pt.x = integerExpr(mInstance);
  match(mInstance,COMMA);
  pt.y = integerExpr(mInstance);
  match(mInstance,COMMA);
  rx = integerExpr(mInstance);
  match(mInstance,COMMA);
  ry = integerExpr(mInstance);
...
void doArc(MINIBASIC *mInstance) {
  ...
  match(mInstance,ARC);
  x1 = integerExpr(mInstance);
  match(mInstance,COMMA);
  y1 = integerExpr(mInstance);
  match(mInstance,COMMA);
  x2 = integerExpr(mInstance);
  match(mInstance,COMMA);
  y2 = integerExpr(mInstance);
...
int16_t integerExpr(MINIBASIC *mInstance) {
...
fai_diskspace:
      ...
      break;
		...
		case TEMPERATURE:
			...
			answer = myround(ReadTemperature()/16.0,0.1);    // 0.0625 precisione TC77, v.doc
      break;
		case RSSI:
...
VARIABLE *addNewVar(MINIBASIC *mInstance,const char *id,char type) {
  ...
  switch(mInstance->trace & 0x84) {
    char buf[16];
		case 4:
			sprintf(buf,"{%s}",id);
			myTextOut(mInstance,buf);
			break;
		case 0x84:		// a volte potrebbe andare meglio questo...
			err_printf("{%s}",id);
			break;
    }
...
*/
int16_t integer(MINIBASIC *mInstance,double x) {
...
BSTATIC LINE_NUMBER_STMT_POS_TYPE getNextStatement(MINIBASIC *mInstance,const char *str) {
  ...
  // se questa viene usata da command line, tipo FOR o DO, si schianta... come fare??
  ...
  switch(*str) {
    ...
    case ':':
      ...
      break;
    case '\n':
      //if(!mInstance->script)  esci, o errore ERR_INVALIDDIRECT, per doDo doFor doGoto
      i++;
      str++;
      break;
    }
...
BSTATIC TOKEN_NUM getAToken(MINIBASIC *mInstance) {
  
  return getToken(mInstance->string);
  }
...
int8_t strnicmp(const char *string1, const char *string2, size_t count) {
  ...
  do {
    if(tolower((unsigned char)*string1) != tolower((unsigned char)*string2++))
      return (int8_t)tolower((unsigned char)*string1) -	(int8_t)tolower((unsigned char)*--string2);
    if(!*string1++)
      break;
    } while(--count);
...
const char *strichr(const char *hay, const char needle) {
  ...
  const char needlel=tolower(needle);
  do {
    if(tolower(*hay) == needlel)
      return hay;
    } while(*hay++);
...
static inline __attribute((always_inline)) const char *skipSpaces(const char *str) {
